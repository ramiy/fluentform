...
class Helper
{
    ...
    public static function getAdvancedFilterOptions($formId)
    {
        $form = fluentFormApi()->find($formId);
        $generalCodes = \FluentForm\App\Services\FormBuilder\EditorShortCode::getGeneralShortCodes();
        $submissionCodes = \FluentForm\App\Services\FormBuilder\EditorShortCode::getSubmissionShortcodes();
        $fields = \FluentForm\App\Modules\Form\FormFieldsParser::getInputs($form, ['label', 'element', 'options']);
        ...
        foreach ($fields as $value => $field) {
            ...
            if ($field['element'] == 'input_text') {
                $arr['type'] = 'nullable_text';
            } elseif ($field['element'] == 'input_checkbox') {
                $arr['type'] = 'selections';
                $arr['options'] = $field['options'];
                $arr['option_key'] = 'checkbox';
                $arr['is_multiple'] = true;
                $arr['is_singular_value'] = true;
            } else {
                $arr['type'] = 'numeric';
            }
            $fieldCodes[] = $arr;
        ...
        $generalCodes = self::formatAdvancedFilters($generalCodes);
        $submissionCodes = self::formatAdvancedFilters($submissionCodes);
        
        $groups = [];
        $allCodes = array_merge($formShortCodes, $generalCodes, $submissionCodes);
    ...
    public static function formatAdvancedFilters($shortCodes)
    {
        $codes = [];
        ...
        return [
            $shortCodes['title'] => [
                ...
                'value'    => sanitize_title($shortCodes['title']),
                'children' => $codes
            ]
        ];
    }
